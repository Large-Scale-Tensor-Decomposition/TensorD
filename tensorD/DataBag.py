...
def gen_core(coreR, max_prime=10000):
    """
    generate core tensor of shape ranks[1] \\times ranks[2] \\times ranks[3]
    generate algrithm:
    return:
       tf.tensor of shape ranks[1] \\times ranks[2] \\times ranks[3]
    """
        ...
        if mode != -1:
            ...
            for ii in range(I_i):
                for jj in range(rank):
                    ...
                    init_mat[mode][ii,jj] = seed / 1334456 * 2

    return init_mat


def rand_list2(shape_list, ranks):
    # initial seed
    seed = 11796
    order = len(shape_list)
    init_mat = [np.zeros(shape=(shape_list[ii], ranks[ii])) for ii in range(order)]
    for mode in range(order):
        if mode != -1:
            I_i = shape_list[mode]
            J_i = ranks[mode]
